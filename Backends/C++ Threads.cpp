		...
		#error "'core/Backends/C++ Threads.c: C++ threads are not installed on the current device.
	#endif
#endif

#define COMMON_CURRENT_PLATFORM COMMON_PLATFORM_CPP_STD
...
int main() {
	...
	COMMON_STD thread threads[GLOBAL_NUMBER_OF_WORKERS]; // TODO: Not ISO-C++-supported
	int data[GLOBAL_NUMBER_OF_WORKERS]; // TODO: Not ISO-C++-supported
	for (int i = 0; i < GLOBAL_NUMBER_OF_WORKERS; ++i) {
		data[i] = i;
		threads[i] = COMMON_STD thread(runWorker, &data[i]);
	}
	for (int i = 0; i < GLOBAL_NUMBER_OF_WORKERS; ++i) threads[i].join();
