/* Backend for running common_seedfinding */
...
int main() {
	...
	if (INPUT_FILEPATH) {
		...
		if (!inputFile) RAISE_EXCEPTION_OR_QUIT("Backends/Pthreads.c: int main(): inputFile = fopen(INPUT_FILEPATH, \"r\"): Failed to open %s.\n", INPUT_FILEPATH);
	}
	if (OUTPUT_FILEPATH) {
		outputFile = fopen(OUTPUT_FILEPATH, "w");
		if (!outputFile) RAISE_EXCEPTION_OR_QUIT("Backends/Pthreads.c: int main(): outputFile = fopen(OUTPUT_FILEPATH, \"w\"): Failed to open %s.\n", OUTPUT_FILEPATH);
	}
